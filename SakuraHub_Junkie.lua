...
Junkie.service = "Sakura Hub"
Junkie.identifier = "1170353"
Junkie.provider = "Sakura Hub"
-- ===========================================================
...
-- Umbral alto (45px) para que sea difícil de mover accidentalmente en móvil
MakeDraggable(QB, 45)
...
QB.MouseButton1Click:Connect(function()
    ...
    if Config.AimbotEnabled then
        QB.Text = "🎯\nON"
        QB.BackgroundColor3 = Color3.fromRGB(255, 110, 150) -- Rosa activo
        QS.Color = Color3.fromRGB(255, 180, 210)
        QB.TextColor3 = Color3.fromRGB(255, 255, 255)
    else
        QB.Text = "🎯\nOFF"
        QB.BackgroundColor3 = Color3.fromRGB(35, 18, 28) -- Oscuro inactivo
        QS.Color = Color3.fromRGB(255, 130, 170)
        QB.TextColor3 = Color3.fromRGB(255, 150, 180)
    end
end)
...
------------------------------------------------------------------------
-- 5. LÓGICA DE VERIFICACIÓN (JUNKIE)
------------------------------------------------------------------------
...
local function VerifyKey(enteredKey, isAutoLogin)
    ...
    local result = Junkie.check_key(enteredKey)
    ...
    if result and result.valid then
        ...
        DeleteSavedKey()
        local errorMsg = (result and (result.error or result.message)) or "Key inválida"
        
        if isAutoLogin then
            StatusText.Text = "⚠️ Key expirada o inválida"
        else
...
-- Botón Get Key (genera link de Junkie)
GetKeyBtn.MouseButton1Click:Connect(function()
    GetKeyBtn.Text = "Generando..."
    ...
    if link then
        if setclipboard then
            ...
            StatusText.Text = "Link copiado al portapapeles"
            StatusText.TextColor3 = Color3.fromRGB(130, 225, 160)
        else
            GetKeyBtn.Text = "Error"
            StatusText.Text = "No se pudo copiar el link"
            StatusText.TextColor3 = Color3.fromRGB(255, 100, 120)
        end
    else
        GetKeyBtn.Text = "Espera..."
        StatusText.Text = (err == "RATE_LIMITED") and "Espera 5 minutos" or "Error al generar link"
        StatusText.TextColor3 = Color3.fromRGB(255, 100, 120)
    end
