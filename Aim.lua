...
local AimPart = "Head"
local FovRadius = 0
local Smoothness = 15 -- Velocidad de seguimiento (Mayor = más rápido y preciso, Menor = más suave)
...
-- ================= RENDER LOOP (ESP + FOV + CAMERA AIMBOT) =================
RunService.RenderStepped:Connect(function(dt)
	...
	if AimbotEnabled then
		...
		if CurrentTarget then
			-- Cálculo de CFrame de cámara suave y preciso basado en DeltaTime
...
-- ================= PHYSICS LOOP (ORIENTACIÓN DEL PERSONAJE) =================
-- Mover la rotación física del RootPart a Stepped previene el stuttering/flicker en 1ª persona
RunService.Stepped:Connect(function()
	if AimbotEnabled and CurrentTarget and RootPart and Character and Character:Parent() then
		...
		if isFirstPerson then
			...
			-- Alineación horizontal limpia sin modificar inclinaciones indeseadas en X/Z
