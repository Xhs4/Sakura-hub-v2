...
local Config = {
    ...
    AimSmoothness = 0.85,   -- MUY FUERTE (casi lock instantáneo)
    AimFOV = 220,           -- FOV grande
    ShowFOV = true,
    WallCheck = true,
    TeamCheck = true,
    OnlyStars = true,
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and p.Character and IsEnemy(p) and not (p.Team and Config.TeamFilter[p.Team.Name] == false) then
            ...
            if part and hum and hum.Health > 0 and IsTargetVisible(part) then
                ...
                if onS and pos.Z > 0 then
                    local dist = (Vector2.new(pos.X, pos.Y) - vc).Magnitude
                    if dist < sd then
                        sd = dist
                        cl = part -- devolvemos la parte directamente
                    end
                end
            end
        end
...
RunService.RenderStepped:Connect(function()
    ...
    if shouldAim then
        ...
        if targetPart then
            ...
            -- 0.85 = muy agresivo, casi instantáneo
