...
local Config = {
    ...
    AimSmoothness = 0.25, AimFOV = 130, ShowFOV = true, WallCheck = true,
    OnlyStars = true, -- ← NUEVO: solo jugadores con estrellas
...
local function getStars(player)
    ...
    -- 3. IntValue / NumberValue directos
    for _, name in ipairs(POSSIBLE_NAMES) do
...
CToggle(PESP, "Only Stars (Wanted)", Config.OnlyStars, function(v) Config.OnlyStars = v end) -- ← NUEVO TOGGLE
...
local CTX = Instance.new("TextLabel") CTX.Size, CTX.BackgroundTransparency, CTX.Text, CTX.TextColor3, CTX.Font, CTX.TextSize, CTX.Parent = UDim2.new(1,0,0,40), 1, "🌸 SAKURA HUB + Stars\nCreated by: ColorOzz + Stars System", Color3.fromRGB(255,200,220), Enum.Font.FredokaOne, 11, PCrd
...
------------------------------------------------------------------------
-- 4. ESP & AIMBOT ENGINE (con filtro de estrellas)
------------------------------------------------------------------------
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aimbot (mejorado con filtro de estrellas)
