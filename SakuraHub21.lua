...
local FOVF = Instance.new("Frame") FOVF.Size, FOVF.BackgroundColor3, FOVF.Parent = UDim2.new(1,0,0,30), Color3.fromRGB(40,22,32), PAim
...
-- System logic fix: Character & Player Tracking
local function AddP(p)
    ...
    p.CharacterRemoving:Connect(function() ESP[p].Ch = nil hl.Adornee = nil end)
end
...
-- Detector Universal de Estrellas (Líder, Stats, PlayerGui, Overhead UI)
local function GetWantedLevel(p)
    local function ParseVal(v)
        if type(v) == "number" then return v end
        if type(v) == "string" then
            local count = 0
            for _ in v:gmatch("⭐") do count = count + 1 end
    ...
    local function SearchFolder(folder)
        ...
        for _, v in ipairs(folder:GetChildren()) do
            ...
            if (name:find("wanted") or name:find("star") or name:find("bounty") or name:find("estrella") or name:find("fugitivo")) then
                if v:IsA("ValueBase") then return ParseVal(v.Value) end
            end
        end
        return nil
    end

    local val = SearchFolder(p:FindFirstChild("leaderstats")) or SearchFolder(p:FindFirstChild("Stats")) or SearchFolder(p:FindFirstChild("Data")) or SearchFolder(p)
    ...
    if p.Character then
        for _, desc in ipairs(p.Character:GetDescendants()) do
            if desc:IsA("TextLabel") or desc:IsA("TextButton") then
                ...
                if txt:find("⭐") then
                    local count = 0
                    for _ in txt:gmatch("⭐") do count = count + 1 end
                    if count > 0 then return count end
...
local function IsCivilian(p)
    ...
    return name:find("civil") or name:find("citizen") or name:find("criminal") or name:find("prisoner") or name:find("rebel")
end
...
local function IsEnemy(p)
    if IsCivilian(LP) then
        return IsPolice(p)
    else
        return IsCivilian(p) and GetWantedLevel(p) > 0
    end
end
...
local function GetClosest()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and (p.Character or workspace:FindFirstChild(p.Name)) and IsEnemy(p) then
            local char = p.Character or workspace:FindFirstChild(p.Name)
            local part, hum = char:FindFirstChild(Config.AimPart), char:FindFirstChildOfClass("Humanoid")
...
RunService.RenderStepped:Connect(function()
    if (Config.AimMode == "Mobile (Bubble)" and Config.AimLockEnabled) or (Config.AimMode == "PC (Right Click)" and IsAimingRightClick) then
        local t = GetClosest()
        if t then
            local char = t.Character or workspace:FindFirstChild(t.Name)
            if char and char:FindFirstChild(Config.AimPart) then
                Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, char[Config.AimPart].Position), Config.AimSmoothness)
            end
        end
    end

    local lc = LP.Character or workspace:FindFirstChild(LP.Name)
    local lr = lc and (lc:FindFirstChild("HumanoidRootPart") or lc:FindFirstChild("Torso"))
    ...
    for p, d in pairs(ESP) do
        ...
        if char and root and head and lr and IsEnemy(p) then
            ...
            if onS then
                ...
                if Config.ESPEnabled then
                    ...
                    if Config.ShowWantedStars and IsCivilian(p) then
                        local stars = GetWantedLevel(p)
                        if stars > 0 then
                            local starStr = string.rep("⭐", math.min(stars, 6))
                            strText = strText .. " | " .. starStr
                        end
                    end
                ...
                if Config.HighlightsEnabled then d.H.Adornee = char d.H.FillColor, d.H.Enabled = col, true else d.H.Enabled = false end
