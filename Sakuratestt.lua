...
local IsAuthenticated = false -- Bloqueo de seguridad

local httpRequest = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request

-- Funciones del Sistema de Guardado Local
...
SplashFrame.Size, SplashFrame.Position, SplashFrame.BackgroundColor3, SplashFrame.BackgroundTransparency, SplashFrame.Parent = UDim2.new(1,0,1,0), UDim2.new(0,0,0,0), Color3.fromRGB(12, 6, 10), 0.2, ScreenGui
...
SplashSubText.Size, SplashSubText.Position, SplashSubText.BackgroundTransparency, SplashSubText.Text, SplashSubText.TextColor3, SplashSubText.Font, SplashSubText.TextSize, SplashSubText.Parent = UDim2.new(1,0,0,20), UDim2.new(0,0,0,125), 1, "Loading resources...", Color3.fromRGB(200, 150, 175), Enum.Font.GothamMedium, 10, SplashCard

------------------------------------------------------------------------
-- 2. LOGIN PANEL (TRANSPARENTE Y ESTÉTICA SAKURA)
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- 3. INTERFAZ PRINCIPAL Y SCRIPT DE CHEATS (OCULTO/DESACTIVADO)
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- 4. LÓGICA DEL MOTOR (ESP Y AIMBOT EN CERO HASTA AUTENTICAR)
------------------------------------------------------------------------
...
-- Loop de renderizado estrictamente protegido
...
------------------------------------------------------------------------
-- 5. LÓGICA DE VALIDACIÓN Y AUTO-LOGIN
------------------------------------------------------------------------
local function UnlockScript()
    IsAuthenticated = true
    StartCheatEngine()
    
    TweenService:Create(SplashBlur, TweenInfo.new(0.5), {Size = 0}):Play()
...
------------------------------------------------------------------------
-- 6.SECUENCIA DE INICIO PASO A PASO (SPLASH -> LOGIN / AUTO-LOGIN)
------------------------------------------------------------------------
task.spawn(function()
    -- Paso 1: Carga animada del Splash Screen
    SplashSubText.Text = "Loading resources..."
    TweenService:Create(SplashBarFill, TweenInfo.new(1.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0.6, 0, 1, 0)}):Play()
    task.wait(1.2)
    
    SplashSubText.Text = "Checking key cache..."
    TweenService:Create(SplashBarFill, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0)}):Play()
    task.wait(0.8)
    
    -- Paso 2: Desvanecer la tarjeta del Splash Screen
    TweenService:Create(SplashCard, TweenInfo.new(0.4), {BackgroundTransparency = 1}):Play()
    ...
    -- Paso 3: Verificar auto-login o mostrar panel de Key transparente
