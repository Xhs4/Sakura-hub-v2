...
-- Búsqueda Exhaustiva de Estrellas/Bounty/Wanted
local function GetWantedLevel(p)
    local function CheckObj(obj)
        if not obj then return nil end
        for _, v in ipairs(obj:GetChildren()) do
            local name = v.Name:lower()
            if (name:find("wanted") or name:find("star") or name:find("bounty") or name:find("estrella")) and (v:IsA("ValueBase") or v:IsA("NumberValue") or v:IsA("IntValue")) then
    ...
    local val = CheckObj(p:FindFirstChild("leaderstats")) or CheckObj(p:FindFirstChild("Stats")) or CheckObj(p:FindFirstChild("Data")) or CheckObj(p)
...
local function IsPolice(p)
    ...
    return name:find("police") or name:find("cop") or name:find("guard") or name:find("policía") or name:find("policia")
end

local function IsCivilian(p)
    if not p.Team then return true end
    local name = p.Team.Name:lower()
    return name:find("civil") or name:find("citizen") or name:find("criminal")
end

local function IsEnemy(p)
    local myIsPoly = IsPolice(LP)
    local targetIsPoly = IsPolice(p)
    local targetIsCiv = IsCivilian(p)
    local stars = GetWantedLevel(p)

    if myIsPoly then
        return targetIsCiv and stars > 0
    else
        return targetIsPoly
    end
end
