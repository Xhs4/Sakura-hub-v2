...
local Config = {
    ...
    AimSmoothness = 0.25, -- AimLock recalibrado para respuesta directa e instantánea sin desvíos
    AimFOV = 130, ShowFOV = true, WallCheck = true,
    ...
    if (Config.AimMode == "Mobile (Bubble)" and Config.AimLockEnabled) or (Config.AimMode == "PC (Right Click)" and IsAimingRightClick) then
        ...
        if t and t.Character then
            ...
            if targetPart then
                ...
                -- Se calcula la rotación exacta hacia el centro de la pieza seleccionada
                local targetCFrame = CFrame.new(currentCamPos, targetPos)
                
                -- Lerp recalibrado: suavizado óptimo sin retraso para evitar disparos desviados
