...
local Config = {
    ...
    AimbotEnabled = false, AimMode = "Mobile (Bubble)", AimPart = "Head",
    AimSmoothness = 1,      -- LOCK TOTAL (mira pegada al objetivo)
    AimFOV = 250,
    ShowFOV = true,
    WallCheck = true,
    TeamCheck = true,
...
local TBT = Instance.new("TextLabel") TBT.Size, TBT.Position, TBT.BackgroundTransparency, TBT.Text, TBT.TextColor3, TBT.Font, TBT.TextSize, TBT.TextXAlignment, TBT.Parent = UDim2.new(1,-90,1,0), UDim2.new(0,12,0,0), 1, "🌸 SAKURA HUB + Stars", Color3.fromRGB(255,192,210), Enum.Font.FredokaOne, 13, Enum.TextXAlignment.Left, TB
...
CToggle(PESP, "Only Stars (Wanted)", Config.OnlyStars, function(v) Config.OnlyStars = v end)

CToggle(PAim, "Show FOV Circle", Config.ShowFOV, function(v) Config.ShowFOV, FOV.Visible = v, v end)
CToggle(PAim, "Wall Check", Config.WallCheck, function(v) Config.WallCheck = v end)
CToggle(PAim, "Team Check (no teammates)", Config.TeamCheck, function(v) Config.TeamCheck = v end)
...
local CTX = Instance.new("TextLabel") CTX.Size, CTX.BackgroundTransparency, CTX.Text, CTX.TextColor3, CTX.Font, CTX.TextSize, CTX.Parent = UDim2.new(1,0,0,50), 1, "🌸 SAKURA HUB + Stars\nAimbot Strong + Team/Wall Check", Color3.fromRGB(255,200,220), Enum.Font.FredokaOne, 11, PCrd
local DCB = Instance.new("TextButton") DCB.Size, DCB.Position, DCB.BackgroundColor3, DCB.Text, DCB.TextColor3, DCB.Font, DCB.TextSize, DCB.Parent = UDim2.new(1,0,0,32), UDim2.new(0,0,0,55), Color3.fromRGB(255,110,150), "Copy Discord Invite", Color3.fromRGB(255,255,255), Enum.Font.GothamBold, 10, PCrd
...
local function IsEnemy(p)
    ...
    if Config.TeamCheck and p.Team == LP.Team then return false end
...
local function GetTargetPart(char)
    ...
    -- Prioridad: Head -> UpperTorso -> HumanoidRootPart (funciona también en vehículos)
...
local function GetClosest()
    local bestPart, bestDist = nil, Config.AimFOV
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if p.Team and Config.TeamFilter[p.Team.Name] == false then continue end
        if Config.OnlyStars and getStars(p) <= 0 then continue end
...
UserInputService.InputBegan:Connect(function(i)
    if IsAuthenticated and i.UserInputType == Enum.UserInputType.MouseButton2 and Config.AimMode == "PC (Right Click)" then
...
RunService.RenderStepped:Connect(function()
    ...
    -- AIMBOT LOCK DURO (mira pegada, sigue al objetivo incluso en vehículos)
    local shouldAim = (Config.AimMode == "Mobile (Bubble)" and Config.AimbotEnabled)
                   or (Config.AimMode == "PC (Right Click)" and IsAimingRightClick)

    if shouldAim then
        local targetPart = GetClosest()
        if targetPart and targetPart.Parent then
            -- Lock total: la cámara se queda mirando exactamente al objetivo cada frame
    ...
    for p, d in pairs(ESP) do
        ...
        local hasStars = getStars(p) > 0
        local passStars = (not Config.OnlyStars) or hasStars
        local passTeam = IsEnemy(p) and not (p.Team and Config.TeamFilter[p.Team.Name] == false)

        if char and root and head and lr and passStars and passTeam then
            local pos, onS = Camera:WorldToViewportPoint(root.Position)
            if onS then
                local stars = getStars(p)
                local col = hasStars and Color3.fromRGB(255, 50, 50) or (p.TeamColor and p.TeamColor.Color or Color3.fromRGB(255,110,150))
                ...
                if Config.ESPEnabled then
                    ...
                    local nameStr = p.DisplayName
                    if stars > 0 then
                        nameStr = nameStr .. " ★" .. stars
                    end
                    d.T.Text = string.format("%s [%d m]", nameStr, math.floor((lr.Position - root.Position).Magnitude))
                    d.T.Color = col
                ...
                if Config.HighlightsEnabled then d.H.FillColor, d.H.OutlineColor, d.H.Enabled = col, col, true else d.H.Enabled = false end
