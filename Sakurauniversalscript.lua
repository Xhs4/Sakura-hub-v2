...
MF.Size, MF.Position, MF.BackgroundColor3, MF.Active, MF.Visible, MF.Parent = UDim2.new(0,350,0,300), UDim2.new(0.5,-175,0.5,-150), Color3.fromRGB(22,12,18), true, false, ScreenGui
...
local PESP, PAim, PTms, PCrd = Instance.new("ScrollingFrame"), Instance.new("Frame"), Instance.new("ScrollingFrame"), Instance.new("Frame")
...
PESP.Visible, PESP.ScrollBarThickness, PTms.ScrollBarThickness = true, 3, 3
...
local function CToggle(parent, name, def, cb)
    local f = Instance.new("Frame") f.Size, f.BackgroundColor3, f.Parent = UDim2.new(1,-6,0,28), Color3.fromRGB(40,22,32), parent
...
-- Opciones del menú ESP ampliado
CToggle(PESP, "Master ESP", Config.ESPEnabled, function(v) Config.ESPEnabled = v end)
CToggle(PESP, "ESP Box", Config.Box, function(v) Config.Box = v end)
CToggle(PESP, "ESP Chams", Config.ESPChams, function(v) Config.ESPChams = v end)
CToggle(PESP, "Health Bar", Config.HealthBar, function(v) Config.HealthBar = v end)
CToggle(PESP, "Skeleton", Config.Skeleton, function(v) Config.Skeleton = v end)
CToggle(PESP, "Player Name", Config.PlayerName, function(v) Config.PlayerName = v end)
CToggle(PESP, "Distance", Config.Distance, function(v) Config.Distance = v end)
...
local function AddP(p)
    ...
    healthBarOutline.Thickness, healthBarOutline.Filled, healthBarOutline.Color, healthBarOutline.Visible = 1, true, Color3.fromRGB(0, 0, 0), false
    
    local healthBar = Drawing.new("Square")
    healthBar.Thickness, healthBar.Filled, healthBar.Visible = 1, true, false

    local headCircle = Drawing.new("Circle")
    headCircle.Thickness, headCircle.Filled, headCircle.NumSides, headCircle.Visible = 1.5, false, 16, false
    ...
    local skelPairs = {
        ...
        -- R6 Fallback compatibility
        {"Torso", "Left Arm"}, {"Torso", "Right Arm"}, {"Torso", "Left Leg"}, {"Torso", "Right Leg"}, {"Head", "Torso"}
    }
    for i = 1, #skelPairs do
        local l = Drawing.new("Line")
        l.Thickness, l.Transparency, l.Visible = 1.2, 1, false
        table.insert(skelLines, {Line = l, P1 = skelPairs[i][1], P2 = skelPairs[i][2]})
    end

    -- Sistema de marcación (Sakura Hub)
    local hl = Instance.new("Highlight") 
    hl.FillTransparency, hl.OutlineTransparency, hl.Enabled = 0.6, 0.2, false
    
    ESP[p] = {
        Ch = p.Character, B = box, L = line, T = txt, 
        HBO = healthBarOutline, HB = healthBar, HC = headCircle,
        Skel = skelLines, H = hl
    }
...
local function RemP(p)
    ...
    if d.HBO then d.HBO:Remove() end
    if d.HB then d.HB:Remove() end
    if d.HC then d.HC:Remove() end
    if d.Skel then for _, s in ipairs(d.Skel) do s.Line:Remove() end end
...
-- Sistema de Marcación / Team Check de Sakura Hub
...
RunService.RenderStepped:Connect(function()
    ...
    if (Config.AimMode == "Mobile (Bubble)" and Config.AimbotEnabled) or (Config.AimMode == "PC (Right Click)" and IsAimingRightClick) then
        local t = GetClosest()
    ...
    for p, d in pairs(ESP) do
        ...
        if char and root and head and hum and lr and IsEnemy(p) and not (p.Team and Config.TeamFilter[p.Team.Name] == false) and hum.Health > 0 then
            local pos, onS = Camera:WorldToViewportPoint(root.Position)
            if onS and Config.ESPEnabled then
                ...
                -- Text (Name / Distance)
                local infoTxt = ""
                if Config.PlayerName then infoTxt = p.Name end
                if Config.Distance then 
                    local distM = math.floor((lr.Position - root.Position).Magnitude)
                    infoTxt = infoTxt ~= "" and string.format("%s [%dm]", infoTxt, distM) or string.format("[%dm]", distM)
                end

                if infoTxt ~= "" then
                    local hp = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 1.6, 0))
                    d.T.Text = infoTxt
                    d.T.Position = Vector2.new(hp.X, hp.Y - 14)
                    d.T.Visible = true
                ...
                if Config.HealthBar then
                    ...
                    local barHeight = math.floor(h * healthPct)
                    
                    d.HBO.Position = Vector2.new(x - 6, y)
                    d.HBO.Size = Vector2.new(3, h)
                    d.HBO.Visible = true

                    d.HB.Color = Color3.fromRGB(255, 0, 0):Lerp(Color3.fromRGB(0, 255, 0), healthPct)
                    d.HB.Position = Vector2.new(x - 5, y + (h - barHeight))
                    d.HB.Size = Vector2.new(1, barHeight)
                    d.HB.Visible = true
                else d.HBO.Visible, d.HB.Visible = false, false end

                -- Head Circle
                local headPos, headOnS = Camera:WorldToViewportPoint(head.Position)
                if headOnS then
                    d.HC.Color = col
                    d.HC.Position = Vector2.new(headPos.X, headPos.Y)
                    d.HC.Radius = math.clamp(sc * 0.8, 2, 18)
                    d.HC.Visible = true
                else d.HC.Visible = false end

                -- Skeleton
                if Config.Skeleton then
                    for _, s in ipairs(d.Skel) do
                        local p1 = char:FindFirstChild(s.P1)
                        local p2 = char:FindFirstChild(s.P2)
                        if p1 and p2 then
                            local pos1, vis1 = Camera:WorldToViewportPoint(p1.Position)
                            local pos2, vis2 = Camera:WorldToViewportPoint(p2.Position)
                            if vis1 and vis2 then
                                s.Line.Color = col
                                s.Line.From = Vector2.new(pos1.X, pos1.Y)
                                s.Line.To = Vector2.new(pos2.X, pos2.Y)
                                s.Line.Visible = true
                ...
                if Config.TracersEnabled then
                    ...
                    d.L.To = Vector2.new(pos.X, pos.Y)
                    d.L.Visible = true
                else d.L.Visible = false end

                -- Sakura Hub Original Highlight (Marcación)
                ...
                d.B.Visible, d.T.Visible, d.L.Visible, d.H.Enabled = false, false, false, false
                d.HBO.Visible, d.HB.Visible, d.HC.Visible = false, false, false
                for _, s in ipairs(d.Skel) do s.Line.Visible = false end
            end
        else
            d.B.Visible, d.T.Visible, d.L.Visible, d.H.Enabled = false, false, false, false
...
------------------------------------------------------------------------
-- VERIFICACIÓN Y SECUENCIA DE ARRANQUE
------------------------------------------------------------------------
...
local function VerifyKey(enteredKey, isAutoLogin)
    ...
    if isValid then
        ...
        DeleteSavedKey()
        StatusText.Text = isAutoLogin and "⚠️ Key expirada, ingresa una nueva" or "❌ Key inválida"
