...
-- ===== VARIABLES DE PERSONAJE DINÁMICAS =====
...
local AimPart = "Head"
local FovRadius = 0
local Smoothness = 0.35 -- Factor de suavizado para evitar cam-snapping brusco
...
local function visible(pos)
	...
	params.FilterType = Enum.RaycastFilterType.Exclude -- API actualizada
...
-- ===== LOOP CONTROLADOR DE AIMBOT & CÁMARA/PERSONAJE =====
RunService.RenderStepped:Connect(function()
	if not AimbotEnabled then return end
	
	local target = getBestTarget()
	if target and RootPart and Humanoid then
		local targetPos = target.Position
		
		-- 1. DETECTAR PRIMERA/TERCERA PERSONA
		local isFirstPerson = (Camera.CFrame.Position - Camera.Focus.Position).Magnitude < 1
		
		-- 2. ORIENTAR LA CÁMARA HACIA EL ENEMIGO
		local targetCameraCFrame = CFrame.new(Camera.CFrame.Position, targetPos)
		Camera.CFrame = Camera.CFrame:Lerp(targetCameraCFrame, Smoothness)
		
		-- 3. ORIENTAR EL PERSONAJE EN EL EJE Y (HORIZONTAL)
		-- Evita rotar los pies hacia arriba o hacia abajo si el enemigo está volando o agachado
		local lookAtHorizontal = Vector2.new(targetPos.X - RootPart.Position.X, targetPos.Z - RootPart.Position.Z)
		if lookAtHorizontal.Magnitude > 0 then
			local yaw = math.atan2(-lookAtHorizontal.X, -lookAtHorizontal.Y)
			local newRootCFrame = CFrame.new(RootPart.Position) * CFrame.Angles(0, yaw, 0)
			
			if isFirstPerson then
				-- En 1ª Persona forzamos la orientación instantánea para evitar desincronización
				RootPart.CFrame = newRootCFrame
			else
				-- En 3ª Persona interpolamos suavemente la rotación del cuerpo
				RootPart.CFrame = RootPart.CFrame:Lerp(newRootCFrame, Smoothness)
			end
		end
	end
end)